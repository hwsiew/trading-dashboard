...
import OrdersBook from 'classes/OrdersBook';
import {
  Options,
  OnMessageMeta,
  OnOrderBookMessage,
...
const Luno = (async function(options: Options){
  ...
  const onCloseProvider = (pair: CurrencyPair) => {
    ...
    return (ws: WebSocket) => () => {
      ...
      ticker.order_book = new OrdersBook();
      setTimeout(function(){
  ...
  const onMessageProvider = (pair: CurrencyPair) => {
    ...
    return (ws: WebSocket) => (data: WebSocket.RawData) => {
      ...
      const { last_sequence, order_book } = ticker;
      ...
      if ('status' in message) {
        ...
        const asks: Order[] = update.asks.map(ask => ({ ...ask, type: OrderType.Ask}));
        const bids: Order[] = update.bids.map(bid => ({ ...bid, type: OrderType.Bid}));
        ...
        if(update.create_update){
          const order = update.create_update;
          order_book.add([{ 
            id: order.order_id, 
            type: order.type, 
            price: order.price,
        ...
        if(update.trade_updates.length){
          update.trade_updates.forEach(trade => {
            order_book.trade(trade.maker_order_id, trade.base);
          });
        }
      }

      event_lord.emit(`${pair}:ordersbook`, {
        asks: order_book.getTopAsks(10),
        bids: order_book.getTopBids(10)
      });
    };
  }
  ...
  return {
    stream: async function ( ) {
      for(const { id: pair, candles } of pairs){
        tickers.set(pair, {
          order_book: new OrdersBook<Order>(),
          last_sequence: 0
        });
